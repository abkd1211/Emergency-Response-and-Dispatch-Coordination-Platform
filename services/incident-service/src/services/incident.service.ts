...
export class IncidentService {
  ...
  async listIncidents(
    page = 1,
    limit = 20,
    filters: { status?: IncidentStatus; type?: IncidentType } = {}
  ...
  async listOpenIncidents() {
    const cached = await redisClient.get(REDIS_KEYS.openIncidents());
    if (cached) return JSON.parse(cached);
    ...
    await redisClient.setEx(REDIS_KEYS.openIncidents(), 30, JSON.stringify(incidents));
    return incidents;
  }
